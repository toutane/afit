...
let crt_solver m l y =
  ...
  in
  let partialProductList = initPartialProductList l in
  let rec getIeme i = function
    | [] -> invalid_arg "list too short"
    | e :: l -> if i = 1 then e else getIeme (i - 1) l
  in
  let rec initModularInverseList i = function
    | [] -> []
    | p :: l ->
        let d, _, _ = bezout (getIeme i partialProductList) p in
        if d != 1 then
          modulo (getIeme i partialProductList) p
          :: initModularInverseList (i + 1) l
        else d :: initModularInverseList (i + 1) l
  in
  let inverseList = initModularInverseList 1 l in
  let rec sum i = function
    | [] -> 0
    | rem :: l ->
        (getIeme i partialProductList * getIeme i inverseList * rem)
        + sum (i + 1) l
  in
  let sum = sum 1 y in
  modulo sum m
