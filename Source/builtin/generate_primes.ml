...
let init_eratosthenes n =
  let rec add_to_list x =
    match x with x when x + 2 > n -> [ x ] | x -> x :: add_to_list (x + 2)
  in
  if n < 3 then [ n ] else 2 :: add_to_list 3
...
let eratosthenes n =
  let supp_divisor n l =
    let rec list_browsing = function
      | [] -> []
      | e :: l ->
          if modulo e n = 0 then list_browsing l else e :: list_browsing l
    in
    list_browsing l
  in
...
let double_primes limit isprime =
  let rec build_list = function
    ...
    | x ->
        if isprime x then
          let dp = (x * 2) + 1 in
          if isprime dp then (x, dp) :: build_list (x + 1)
          else build_list (x + 1)
        else build_list (x + 1)
  in
  build_list 2
...
let twin_primes limit isprime =
  let rec build_list = function
    ...
    | x ->
        if isprime x then
          let twin = x + 2 in
          if isprime twin then (x, twin) :: build_list (x + 1)
          else build_list (x + 1)
        else build_list (x + 1)
  in
  build_list 2
